...
const Gmail = () => {
	...
	let info = useSelector((state) => state.product.infoCategory[4])

	const handleBuyGmail = async (count) => {
		try {
			const res = await axios.get(
				`/product/buy_gmail?name=${name}&number=${count['']}`,
				{
					headers: {
